...
function M.setup(programs)
  ...
  hl.bind(mainMod .. " + CTRL + slash",   hl.dsp.exec_cmd("kitty --hold -e dev -i -C tmuxls"), { description = "Dev shell (tmux sessions)" })
  hl.bind(mainMod .. " + SHIFT + Return", hl.dsp.exec_cmd("env NO_DEV_SHELL=1 kitty"),     { description = "Terminal (no devshell)" })
  hl.bind(mainMod .. " + SHIFT + R",      hl.dsp.exec_cmd("kitty -e dev -i -C 'orgmrnc find'"), { description = "Run recent command" })
  hl.bind(mainMod .. " + SHIFT + P",      hl.dsp.exec_cmd(program("piPrompt", "kitty --hold -e pi")), { description = "Pi prompt" })
