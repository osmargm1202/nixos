...
func menuItems() map[string][]Item {
	return map[string][]Item{
		"main": {
			item("󰀻 Apps", "hypr-apps-menu"), item("󰒓 Tools", "hypr-tools-menu"), item("󱐋 Performance", "hypr-performance-menu"), item("󰖩 WiFi", "hypr-wifi-menu"), item("󰂯 Bluetooth", "hypr-bluetooth-menu"), item("󰍉 Search", "orgm-hypr", "smart-run", "run"), item("󰌌 Keybinds", "hypr-keybindings-help"), item("󰒓 System", "hypr-system-menu"), item("󰑓 Reload Dock", "orgm-hypr", "dock", "start", "reload"), item("󰐥 Power", "hypr-power-menu"), item("󰌌 Keyboard", "hypr-keyboard-menu"), item("󰣆 Web App Maker", "hypr-webapp-maker"), item("󰅖 Quit", "", ""),
		},
		"system":      {item("󰑓 Reload Hyprland", "hyprctl", "reload"), item("󰑓 Restart Waybar", "sh", "-lc", "pkill -KILL -f '[o]rgm-hypr waybar watch' || true; pkill -KILL -f '(^|/)waybar($| )|[.]waybar-wrapped' || true; nohup orgm-hypr waybar watch \"$HOME/.config/waybar-hypr\" >/dev/null 2>&1 &"), item("󰑓 Reload Dock", "hypr-system-menu"), item("󰆍 User logs", "kitty", "--class", "user-logs", "-e", "journalctl", "--user", "-n", "200", "--no-pager"), item("󰁯 dot.sh status", "kitty", "--class", "dot-status", "-e", "sh", "-lc", "orgm-dot status --host ${HYPR_HOST:-$(hostname)}; read -r -p 'press enter...'"), item("󰕚 dot.sh diff", "kitty", "--class", "dot-diff", "-e", "sh", "-lc", "orgm-dot diff --host ${HYPR_HOST:-$(hostname)}; read -r -p 'press enter...'"), item("󰑓 dot.sh sync", "kitty", "--class", "dot-sync", "-e", "sh", "-lc", "orgm-dot sync --host ${HYPR_HOST:-$(hostname)}; read -r -p 'press enter...'"), item("󰉋 Open dotfiles", "nautilus", "~/Hobby/dotfiles"), item("󰅖 Cancel", "", "")},
		},
		"tools":       {item("󰆍 Terminal", "kitty"), item("󰉋 Files", "nautilus"), item("󰍉 Search files", "hypr-rofi-open-file"), item("󰃭 Calculator", "gnome-calculator"), item("󰍹 Displays", "nwg-displays"), item("󰸉 Wallpaper next", "hypr-random-wallpaper", "next"), item("󰅖 Cancel", "", "")},
...
func KeybindingCategories() []KeybindingCategory {
	return []KeybindingCategory{
		{ID: "launchers", Title: "Launchers", Icon: "󰀻", Entries: []KeybindingEntry{{"Win+/", "Atajos Hyprland", "hypr-keybindings-help"}, {"Win+Enter", "Terminal", "kitty"}, {"Win+Space", "Menú principal", "hypr-app-launcher"}, {"Win+A", "Buscar / ChatGPT", "orgm-hypr smart-run run"}, {"Win+E", "Explorador de archivos", "orgm-hypr file open-dir --launcher fuzzel"}}},
		...
		{ID: "system", Title: "Sistema", Icon: "󰒓", Entries: []KeybindingEntry{{"Win+P", "Pantallas", "nwg-displays"}, {"Win+Shift+W", "Elegir wallpaper", "hypr-wallpaper-picker"}, {"Win+Shift+C", "Calendario", "orgm-hypr calendar toggle-ui"}, {"Win+Alt+E", "Power menu", "wlogout"}}},
	}
}
