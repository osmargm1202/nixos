...
// Apply writes active theme files, persists current theme state, saves the
// outgoing global and per-monitor wallpapers, then restores the incoming global
// wallpaper followed by its per-monitor overrides.
...
func saveCurrentWallpaperForTheme(stateHome, themeName string) error {
	...
	if err := saveMonitorWallpaperStates(stateHome, themeName); err != nil {
		return err
	}
	path := readTrimmedFile(filepath.Join(stateHome, "hypr-wallpaper", "current"))
	if path == "" {
		return nil
	}
	wallpaperDir := filepath.Join(stateHome, "orgm-theme", "wallpapers")
	return atomicWriteString(filepath.Join(wallpaperDir, themeName+".state"), "path="+path+"\n", 0o644)
}
...
func restoreWallpaperCommands(stateHome, themeName string) []Command {
	wallpaperDir := filepath.Join(stateHome, "orgm-theme", "wallpapers")
	commands := make([]Command, 0)
	if path := readStateFile(filepath.Join(wallpaperDir, themeName+".state"))["path"]; path != "" {
		commands = append(commands, Command{Name: "hypr-wallpaper", Args: []string{"set", path}})
	}
	return append(commands, monitorWallpaperCommands(filepath.Join(wallpaperDir, themeName+".monitors"))...)
}
...
func monitorWallpaperCommands(dir string) []Command {
	...
	for _, entry := range entries {
		...
		if path := readStateFile(filepath.Join(dir, entry.Name()))["path"]; path != "" {
			output := strings.TrimSuffix(entry.Name(), ".state")
			commands = append(commands, Command{Name: "hypr-wallpaper", Args: []string{"set", path, "--monitor", output}})
		}
	}
	return commands
}
