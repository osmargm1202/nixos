...
grep -Eq '^[[:space:]]+thunar[[:space:]]*$' "$I3_PROFILE" ||
...
for profile in \
	...
	nixos/profiles/labwc.nix; do
	grep -Fq 'services.gvfs.enable = true;' "$ROOT/$profile" ||
		fail "GVfs file-manager integration must remain enabled in $profile"
done
...
printf 'PASS: GOA GTK and file-manager integration remain configured\n'
