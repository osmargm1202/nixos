...
do
  nix eval --json "path:$ROOT#nixosConfigurations.${configuration}.config.services.sunshine" |
...
do
  nix eval --json "path:$ROOT#nixosConfigurations.${configuration}.config.services.sunshine.enable" |
    jq -e '. == false' >/dev/null ||
