...
grep -A1 -F 'match = "!focused && !group_focused";' <<<"$picom_config" | grep -Fq 'opacity = 0.93;' ||
  fail 'inactive transparency is not rule-based'
grep -Fq 'match = "focused || group_focused";' <<<"$picom_config" || fail 'active transparency rule missing'
grep -A1 -F 'match = "focused || group_focused";' <<<"$picom_config" | grep -Fq 'opacity = 0.98;' ||
  fail 'active transparency is not rule-based'
...
' <<<"$picom_config")"
grep -Fq 'fade = false;' <<<"$lock_rule" || fail 'lock screen can fade over visible desktop'
grep -Fq 'duration = 0.001;' <<<"$lock_rule" || fail 'lock screen animations are not effectively instant'
...
grep -Eq 'background #[0-9a-fA-F]{8}' "$CONFIG" || fail 'i3bar background lacks alpha channel'
...
jq -e '
  (.cycle | length) > 0 and
  all(.cycle[]; (.bg | test("^#[0-9A-Fa-f]{8}$"))) and
